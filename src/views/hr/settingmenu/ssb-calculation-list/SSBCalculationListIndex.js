...
import SearchSSBCalculationList from "./SearchSSBCalculationList";
import SSBCalculationListTable from "./SSBCalculationListTable";
...
function LegacyWelcomeClass({ t, i18n }) {
  ...
  const [methodsState, setMethodsState] = useState();     // For overtime name dropdown toggle
  ...
  */
  ...
  const loadMethodsAPI = async () => {
    ...
    if (response.flag === false) {
      ...
      setMethodAPI(response.data.data);
    }
  }

  let methodsChange = (e) => {
    setMethodsState(e.target.value);
  }
  ...
  const fnSearch = useCallback => {
    ...
    if (arrMsg.length > 0) {
      ...
      let request = {
        ...
        "company_id": ApiPath.companyID,
        "based_on_methods_id": methodsState,
  ...
  const searchAPI = async (request, pageNumber = 1, msg = true) => {
    ...
    let obj = { package_name: 'hr', url: ApiPath.ssbCalculateMethodListSearch, method: 'post', params };
    setLoading(true);
    let response = await ApiRequest(obj);
    setLoading(false);
    if (response.flag === false) {
      ...
      let res = response.data.ssb_calculation_list;
  ...
  const deleteOK = async () => {
    ...
    if (!isEmpty(deleteIdList)) {
      let url = `${ApiPath.ssbCalculateMethodListDelete}${deleteIdList}?company_id=${ApiPath.companyID}&language=${ApiPath.lang}`;
  ...
  const editRow = (id) => {
    localStorage.setItem('RETURN_CALCULATE_DATA', JSON.stringify(id));
    history.push("./ssb-calculation-method-setup");
  }
  let refresh = () => {
  ...
  return (
    <CRow className="ssbcalculation">
      <CCol xs="12">
        ...
        <CCard>
          <CCardHeader>
            <h5>{t('SSB Calculation List')}</h5>
          </CCardHeader>
          <CCardBody>
            <SearchSSBCalculationList
              ...
              methodAPI={methodAPI} methodsState={methodsState} methodsChange={methodsChange}
              handleFromDateChange={i => setSelectedFromDate(ChangeDate(i))}
              handleToDateChange={i => setSelectedToDate(ChangeDate(i))}
              selectedFromDate={selectedFromDate}
              selectedToDate={selectedToDate}
              searchAPI={() => fnSearch()} />
            <SSBCalculationListTable mainTable={mainTable} rowCount={rowCount} AllCheck={AllCheck} deleteToggleAlert={deleteToggleAlert}
...
export default function SSBCalculationListIndex() {
  return <Welcome />;
}
