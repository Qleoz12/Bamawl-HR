...
/**
 * EmployeeAllowanceRegistrationTable Component use for EmployeeOvertimeRegistrationIndex
 *
 * @author  dh_khanh
 * @create_date
 */
const EmployeeAllowanceRegistrationTable = props => {
    let {
        mainTable,
        rowCount,
        editData,
                        ...
                        <CCol lg="12">
                            {
                                ...
                                <CRow alignHorizontal="end">
                                    <div className="row-count-msg">
                                        <p id="lblTotalRows" className="mb-0">
                            ...
                            <table className="table purchase-order-list" aria-label="simple table">
                                <thead id="thead-id">
                                    <tr width="100%">
                                        ...
                                        <th id="tblEmployeeCode">
                                            <div className="d-flex align-items-baseline">
                                                <CImg src={'avatars/titleicon.png'} className="title-icon" />
                                                {t('Employee Code')}
                                            </div>
                                        </th>
                                        <th id="tblEmployeeName">
                                            <div className="d-flex align-items-baseline">
                                                <CImg src={'avatars/titleicon.png'} className="title-icon" />
                                                {t('Employee Name')}
                                            </div>
                                                    ...
                                                    {i.departments.map((sec, idx) => {
                                                        return (
                                                            <tr key={idx} width="100%">
                                                                {
                                                                    idx === 0 && <>
                                                                        <td width="" className={mainTable.length-1 === index ? "td-no text-right border-bottom-left-radius" : "td-no text-right"} rowSpan={i.departments.length}>
                                                                            {index + 1}
                                                                        </td>
                                                                ...
                                                                {
                                                                    idx === 0 && <>
                                                                        <td width="" className={editData && mainTable.length-1 === index ? "border-bottom-right-radius td-joined-date" : "td-joined-date"} rowSpan={i.departments.length}>
                                                                            {Moment(i.joined_date).format('YYYY-MM-DD')}
                                                                        </td>
                                                                        {
                                                                            !editData &&
                                                                            <td width="" id="removeData" className={mainTable.length-1 === index ? "border-bottom-right-radius" : ""} rowSpan={i.departments.length}>
...
export default EmployeeAllowanceRegistrationTable;
