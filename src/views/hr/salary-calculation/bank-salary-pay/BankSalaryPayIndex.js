...
import { CCol, CRow, CCard, CCardHeader, CCardBody, CLabel } from '@coreui/react';
...
import { isEmpty, notAcceptCharacter } from '../../hr-common/common-validation/CommonValidation'; // Common validation function
...
function LegacyWelcomeClass({ t, i18n }) {
  ...
  const formLoad = async() => {
    let obj = { url: 'api/bank-salary-pay/form-load', method: 'get', params: {
      login_id: loginId,
    ...
    if(response.flag == false){ // catch error
      ...
      if(response.data.transfer_method.length===1){
        setTransferMethodRadio(response.data.transfer_method[0].transfer_method);
      }
      setLoading(false);
    }
  }
  ...
  const handlePaymentMonthChange = (e) => {  formLoad();setAddEmpListData([]);setAddEmpListDataHeader([])
    // ;setCurBankRadio();setTransferMethodRadio();
   ...
   const unique = (data) => {
    ...
    for (var i = 0; i < data.length; i++) {
      if (!(arr).includes(data[i])) {
          ...
          alert(t(message.JSE007).replace('%s',t(`Employee ID ${data[i]}`)));
      }
    }
    return arr;
  }
 ...
 const addBtn = async() =>{
  let old_emp_id=[];let new_emp_id=[];let empData = [];let err = []; 
  setLoading(true);
  let data = empListModalData.filter(data=> data.is_checked === true);
  if(!isEmpty(data)){
    if(!isEmpty(addEmpListData)){
      data.map(x=> {new_emp_id=[...new_emp_id, {"employee_id":x.employee_id,"employee_code":x.employee_code,"employee_name":x.employee_name,"total_amount":x.total_amount}]});
      addEmpListData.map(x=> {
        old_emp_id=[...old_emp_id, {"employee_id":x.employee_id,"employee_code":x.employee_code,"employee_name":x.employee_name,"total_amount":x.total_amount}]
      });
      let union_emp_data = new_emp_id.concat(old_emp_id);
      let id_only = unique(union_emp_data.map(x=> x.employee_id)).sort();
      empData = new_emp_id.filter(a=> !old_emp_id.find(b=> a['employee_id'] === b['employee_id'])).concat(old_emp_id);
      // empData = union_emp_data.filter((e,index)=> id_only[index] == e.employee_id);  // updated data
      empData.sort((a, b)=> {
        return a.employee_id - b.employee_id;
      })
    }else{
      data.map(x=> {empData = [...empData,{"employee_id":x.employee_id,"employee_code":x.employee_code,"employee_name":x.employee_name,"total_amount":x.total_amount} ];})
    }
    ...
    let obj = { url: 'api/bank-salary-pay/add-employee-list', method: 'post'
  , params: {
              "login_id": loginId,
              "company_id": companyId,
    ...
    if(response.flag === false){
      ...
      if(response.data.status == 'NG'){
        ...
        setAddEmpListData(response.data.table_body);
...
/** End employee list modal add button Function */
  ...
  const empListModalClose = ()=>{ 
    ...
    setDeptState(''); setRoleState(''); setEmpID(''); setEmpCode(''); setEmpName('');
  }
  ...
  let allListCheckBoxChange = ()=>{//needed
  ...
  let empListTableDeleteBtn = (e)=>{ 
    let data = addEmpListData.filter(data=> data.employee_id !== e); setAddEmpListData(data); 
  }
  /** End  employee list table delete button Function */
  /** Start search Function */
  let searchBtn = async()=>{   // show employee data with salary ( already salary calculated employee )
    setLoading(true);
    setEmpModalError([]);setError([]);
    let search = {
            "method":"post",
            "url": "api/bank-salary-pay/search-employee-list",
            "params": {
              "login_id":loginId,
              "company_id":companyId,
              "employee_id":empID,
  ...
  let paymentMethodOnChange = async(i) => {
    ...
    let data;
    let id_list  = [];
    data = paymentMethod.map(item =>item.bank_id == i.bank_id ? { ...item, is_checked: checked } : item);
  ...
  let roleChange = (e) =>{ setRoleState(e.target.value); }
  
  /** Start employee list plus button Function */
  const empListPlusBtn = ()=>{ 
    setSuccess([]);setError([]);setEmpModalError([]);
    let errMsg = [];
    if(isEmpty(selectedPaymentMonth)){ let str = t(message.JSE001).replace('%s',t('Payment Month'));errMsg.push(str);}
    if(isEmpty(curBankRadio)){ let str = t(message.JSE019).replace('%s',t('Currency Bank Account'));errMsg.push(str);}
    if(isEmpty(errMsg)){
      setEmpListModalShow(true);
    }else{setError(errMsg);scrollTop();}
  }
  /** End employee list plus button Function */
  
  /** Start Transfer Amount Function */
  let transferAmount = async() => {
    ...
    if(isEmpty(paymentMethodID)){ let str = t(message.JSE019).replace('%s',t('Payment Method'));errMsg.push(str);}
    ...
    if (isEmpty(errMsg)) {
      ...
      Object.size = (obj) => {
        let size = 0, key;
        for (key in obj) { if (obj[key].length != 0) size++;}
        return size;
      };
      let filtered = data.filter((item) => { return Object.size(item.amount_data) < 1; });
      if (filtered.length > 0) {
        let str = "";
        const temp = filtered.map((item) => {return item.employee_id;});
        str = "Employee ID '" + temp.join() + "' do not register in payment transfer setting";
        err.push(str);
        setError(err);
        scrollTop();
      }else {
        ...
        if(!isEmpty(data)){
          for(let i=0; i<data.length; i++){
            let diff_amount = 0; let employee_bank_data = [];
            let total = document.getElementById(`total_${data[i].employee_id}`).value;
            let total_cur_amt = document.getElementById(`total_cur_amt_${data[i].employee_id}`).value;
            if(total != null & total_cur_amt != null){
              if(parseInt(total)>parseInt(total_cur_amt)){
                ...
                err = [...err,t(`Employee ID ${data[i].employee_id} of textbox total amount is less than net salary (Difference amount is ${diff_amount})`)];
              }else if(parseInt(total)<parseInt(total_cur_amt)){
                diff_amount = parseInt(total_cur_amt) - parseInt(total);
                err = [...err,t(`Employee ID ${data[i].employee_id} of textbox total amount is greater than net salary (Difference amount is ${diff_amount})`)];
              }else if(parseInt(total)===parseInt(total_cur_amt)){
          ...
          if(isEmpty(err)){
            ...
            let search = {
              "method":"post",
              "url": "api/bank-salary-pay/transfer-salary",
              "params": {
                "login_id":loginId,
                "company_id":companyId,
                "currency_id":curBankRadio,
            ...
            let response = await ApiRequest(search);scrollTop();
              if(response.flag == false){ // catch error
                setError(response.message);
                formLoad();setAddEmpListData([]);setAddEmpListDataHeader([]);setCurBankRadio();setTransferMethodRadio();setSelectedPaymentMonth(new Date());scrollTop();
              }else{
              if(response.data.status == "OK"){
                setSuccess([response.data.message]);
                formLoad();setAddEmpListData([]);setAddEmpListDataHeader([]);
                // setCurBankRadio();setTransferMethodRadio();
                setSelectedPaymentMonth(new Date());scrollTop();
              }else if(response.data.status == "NG"){
            ...
            setError(err);scrollTop();
          }
        }else{
        let str = t(message.JSE001).replace('%s',t('Data'));
        err.push(str);setError(err);scrollTop();
      }
      setLoading(false);
    }
    }else {
    ...
    addEmpListData.map(data=> data.is_check = false);
    setEmpAllListCheck(false);
    setLoading(false);
  }
  /** End Transfer Amount Function */
  ...
  return (
    <CRow>
      <CCol xs="12">
        ...
        <CCard>
          <CCardHeader><h5><CLabel className="m-0">{t('Bank Salary Pay')}</CLabel></h5></CCardHeader>
          <CCardBody>
            <SettingSetup rowCount={rowCount} rowCountModalBox={rowCountModalBox} selectedPaymentMonth={selectedPaymentMonth} 
            handlePaymentMonthChange={handlePaymentMonthChange} currencyBankAccount={currencyBankAccount} curBankRadio={curBankRadio} currencyBankAccOnChange={currencyBankAccOnChange} paymentMethod={paymentMethod} paymentMethodOnChange={paymentMethodOnChange} transferMethod={transferMethod}  transferMethodRadio={transferMethodRadio} transferMethodOnChange={transferMethodOnChange} empListPlusBtn={empListPlusBtn} addBtn={addBtn} empListModalClose={empListModalClose} empListModalShow={empListModalShow} empListModalData={empListModalData} empAllCheck={empAllCheck} allCheckBoxChange={allCheckBoxChange} subCheckboxChange={subCheckboxChange} 
            empIDData={empIDData} empID={empID} empNameData={empNameData} empName={empName} departmentData={departmentData} searchBtn={searchBtn} empModalError={empModalError} changeAutocomplete={changeAutocomplete} selectAutocomplete={selectAutocomplete}
            empCodeData={empCodeData} empCode={empCode} departmentAPI={departmentAPI} deptState={deptState} deptChange={deptChange} roleAPI={roleAPI} roleState={roleState} roleChange={roleChange} addEmpListData={addEmpListData} empListTableDeleteBtn={empListTableDeleteBtn} allListCheckBoxChange={allListCheckBoxChange} empAllListCheck={empAllListCheck} 
            subListCheckboxChange={subListCheckboxChange} addEmpListDataHeader={addEmpListDataHeader} notAcceptCharacter={notAcceptCharacter} transferAmount={transferAmount}
            />
          </CCardBody>
        </CCard>
      </CCol>
