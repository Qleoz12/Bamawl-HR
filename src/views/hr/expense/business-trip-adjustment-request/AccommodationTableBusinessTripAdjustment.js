...
const AccommodationTableBusinessTripAdjustment = props => {
                  ...
                  <table className="table purchase-order-list" aria-label="simple table">
                    <thead id="thead-id">
                      <tr width="100%">
                        ...
                        <th id="tblDate" className="text-nowrap text-center align-middle" colSpan={2} style={{ borderBottom: "1px solid #fff" }}>
                          <CImg
                            src={"avatars/titleicon.png"}
                            className="imgTitle"
                            alt="no"
                        ...
                        <th id="tblPActualCost" className="text-nowrap align-middle" colSpan={7} style={{ borderBottom: "1px solid #fff" }} >
                          <CImg
                            src={"avatars/titleicon.png"}
                            className="imgTitle"
                            alt="no"
                      ...
                      <tr width="100%">
                        <th className="text-nowrap text-left align-middle no-border-radius">
                          <CImg
                            src={"avatars/titleicon.png"}
                            className="imgTitle"
                            alt="no"
                    ...
                    <tbody >
                        ...
                        data.infor?.map((ele, index) => {
                          return (
                            <tr key={index}>
                              ...
                              <td className="td-date textleft" >
                                {
                                  ele.flag === 0 ? ele.date_out :
                                    <DatePicker value={!isEmpty(ele.date_out) ? ele.date_out : null} change={(value) => handleChangeDateOutTable(value, ele, data, setData)} />
                                }
                              </td>
                                      ...
                                      return (
                                        <tr key={idx}>
                                          <td className="text-left" style={{ border: "none", backgroundColor: '#d5f7df' }}>
                                            ...
                                            <i className="fa fa-times" style={{ cursor: "pointer" }} onClick={() => btnRemoveFileInTable(index, idx, data, setData, "Accommodation")} ></i>
                                          </td>
                                        </tr>
                                      )
                                    })}
                              ...
                              <td className="td-cost textcenter td-actual">
                                {
                                  ele.flag === 0 ? ele.actual_cost.unit_price :
                                    <CInput style={{ textAlign: "right" }} value={ele.actual_cost.unit_price} hidden={ele.flag === 1} name="Unit Price" onChange={(e) => onchangeInput(ele, e, data, setData, "Accommodation")} />
                                }
                              </td>
                              <td className="td-cost textcenter td-actual">
                                {
                                  ele.flag === 0 ? ele.actual_cost.day_times :
                                    <CInput style={{ textAlign: "right" }} value={Number(ele.actual_cost.day_times)} hidden={ele.flag === 1} name="Qty" onChange={(e) => onchangeInput(ele, e, data, setData, "Accommodation")} />
                                }
                              </td>
                              ...
                              <td className="td-cost textcenter td-actual">
                                {
                                  ele.flag === 0 ? ele.actual_cost.fx_rate :
                                    <CInput disabled={ele.actual_cost.unit_price_currency_id == ele.actual_cost.accept_currency_id ? true : false} style={{ textAlign: "right" }} value={ele.actual_cost.fx_rate} hidden={ele.flag === 1} name="FX Rate" onChange={(e) => onchangeInput(ele, e, data, setData, "Accommodation")} />
                                }
                              </td>
                              <td className="td-drop textcenter td-actual">
                                {
                                  ele.flag === 0 ? allCurrency.map(item => item.id == ele.actual_cost.accept_currency_id ? item.currency_desc : "") :
                                    <CSelect value={ele.actual_cost.accept_currency_id} hidden={ele.flag === 1} name={ele.id} onChange={(e) => onchangeAcceptDropdown(e, data, setData, "Accommodation")} >
                                      {allCurrency.map((ele, index) => {
                                        return (
                                          ele.expense_flag === 1 &&
                                          <option key={index} value={ele.id}>{ele.currency_desc}</option>
                              ...
                              <td className="td-action textcenter">
                                <CImg src={'avatars/remove.png'} className="icon-clt" alt="titleicon" onClick={() => btnRemoveRowTable(data, setData, ele, index, "Accommodation")} />
                              </td>
                            </tr>
                          )
                        })
                      ...
                      <tr>
                        <td id="lblSubTotal" style={{ borderBottomLeftRadius: "14px" }} colSpan="6" rowSpan={allCurrency.length}>
                          {t('Sub Total')}
                        </td>
...
export default AccommodationTableBusinessTripAdjustment
