...
const TypeCostBusinessTripRequestTable = props => {
                ...
                <CRow id="table" className="category-border-tfoot">
                  <CCol lg="12">
                    <div className="table-responsive no-border-header">
                      <table
                        className="table purchase-order-list table-category"
                          ...
                          { businessTripList.map((i, index) => {
                              return (
                                  <tr width="100%" key={index}>
                                      ...
                                      {
                                        index!=0&&i.indexValue!=businessTripList[index-1].indexValue&&(
                                          <td
                                              className="text-left text-nowrap td-blue-customer customer-td-img"
                                              rowSpan={businessTripList.filter(e=>e.indexValue==i.indexValue).length}
                                            >
                                              {
                                                businessTripListFile.length>0&&businessTripListFile[index].map((e,indexFile)=>{
                                                  return(
                                                    <div key={indexFile} className="td-img" style={{height:`${100/businessTripListFile[index].length}%`,borderBottom:indexFile!=businessTripListFile[index].length-1?'2px solid white':'none'}}>
                                                        <i className="fas fa-file icon-btn file"></i><span className="text-break ml-3">{e.name.split('/')[e.name.split('/').length-1]}</span>
                        ...
                        <tfoot className="footer">
                          {
                            total.map((e,index)=>{
                              return(
                                <tr key={index}>
                                    {index==0&&(
                                      <td colSpan="4" rowSpan={total.length} className="text-center align-middle td-gray" style={{
                                        borderLeft: "3px solid #858BC3",
                                    }}>{t('Sub Total')}</td>
                                    )}
...
export default TypeCostBusinessTripRequestTable;
